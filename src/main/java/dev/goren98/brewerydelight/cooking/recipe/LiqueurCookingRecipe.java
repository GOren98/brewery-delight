...
public final class LiqueurCookingRecipe implements Recipe<BaseCookingInput> {
    public static final MapCodec<LiqueurCookingRecipe> CODEC= RecordCodecBuilder.mapCodec(i->i.group(
            ...
            Codec.BOOL.optionalFieldOf("homogeneous_materials",false).forGetter(LiqueurCookingRecipe::homogeneousMaterials)
    ...
    public static final StreamCodec<RegistryFriendlyByteBuf,LiqueurCookingRecipe> STREAM_CODEC=new StreamCodec<>(){
        @Override public LiqueurCookingRecipe decode(RegistryFriendlyByteBuf b){int n=b.readVarInt();List<Ingredient> x=new ArrayList<>();for(int j=0;j<n;j++)x.add(Ingredient.CONTENTS_STREAM_CODEC.decode(b));return new LiqueurCookingRecipe(x,ByteBufCodecs.STRING_UTF8.decode(b),ByteBufCodecs.STRING_UTF8.decode(b),ByteBufCodecs.STRING_UTF8.decode(b),ByteBufCodecs.STRING_UTF8.decode(b),b.readVarInt(),b.readVarInt(),b.readBoolean());}
        @Override public void encode(RegistryFriendlyByteBuf b,LiqueurCookingRecipe r){b.writeVarInt(r.ingredients.size());r.ingredients.forEach(x->Ingredient.CONTENTS_STREAM_CODEC.encode(b,x));ByteBufCodecs.STRING_UTF8.encode(b,r.requiredAlcoholType);ByteBufCodecs.STRING_UTF8.encode(b,r.requiredCore);ByteBufCodecs.STRING_UTF8.encode(b,r.coreAlcoholId);ByteBufCodecs.STRING_UTF8.encode(b,r.displayName);b.writeVarInt(r.color);b.writeVarInt(r.cookingTime);b.writeBoolean(r.homogeneousMaterials);}
    };
    private final List<Ingredient> ingredients; private final String requiredAlcoholType,requiredCore,coreAlcoholId,displayName; private final int color,cookingTime; private final boolean homogeneousMaterials;
    public LiqueurCookingRecipe(List<Ingredient> i,String t,String rc,String c,String d,int color,int time,boolean homogeneousMaterials){ingredients=List.copyOf(i);requiredAlcoholType=t;requiredCore=rc;coreAlcoholId=c;displayName=d;this.color=color;cookingTime=Math.max(1,time);this.homogeneousMaterials=homogeneousMaterials;}
    public List<Ingredient> ingredients(){return ingredients;} public String requiredAlcoholType(){return requiredAlcoholType;} public String requiredCore(){return requiredCore;} public String coreAlcoholId(){return coreAlcoholId;} public String displayName(){return displayName;} public int color(){return color;} public int cookingTime(){return cookingTime;} public boolean homogeneousMaterials(){return homogeneousMaterials;}
    ...
    @Override public boolean matches(BaseCookingInput input,Level level){if(ingredients.size()!=4||!(requiredAlcoholType.equals("any")||requiredAlcoholType.equals("brew")||requiredAlcoholType.equals("spirit")))return false;ItemStack a=alcohol(input);if(a.isEmpty()||input.size()!=5)return false;if(a.getOrDefault(ModComponents.AGE.get(),0)!=0||a.getOrDefault(ModComponents.BARREL_LEVEL.get(),0)!=0||!a.getOrDefault(ModComponents.BLEND_AROMAS.get(),Map.<String,Integer>of()).isEmpty())return false;int stage=a.getOrDefault(ModComponents.STAGE.get(),0);if((requiredAlcoholType.equals("brew")&&stage!=1)||(requiredAlcoholType.equals("spirit")&&stage!=2))return false;if(!requiredCore.isEmpty()&&!requiredCore.equals(a.getOrDefault(ModComponents.CORE_ALCOHOL_ID.get(),"")))return false;List<ItemStack> materials=materials(input,a);if(!DirectAlcoholRecipe.shapeless(new BaseCookingInput(materials),ingredients))return false;if(!homogeneousMaterials)return true;ItemStack first=materials.getFirst();for(ItemStack material:materials)if(!material.is(first.getItem()))return false;return true;}
    @Override public ItemStack assemble(BaseCookingInput input,HolderLookup.Provider registries){ItemStack a=alcohol(input);if(a.isEmpty())return ItemStack.EMPTY;List<ItemStack> materials=materials(input,a);var primary=PrimaryAromaCalculator.calculate(materials).orElse(null);String aroma=primary==null?"":primary.aromaId();int level=primary==null?0:Math.min(5,primary.materialCount()+1);return AlcoholStackFactory.create(ModItems.LIQUEUR_BOTTLE.get(),coreAlcoholId,displayName,3,color,aroma,level,AromaUtil.merged(a));}
