...
/** Berries & Cherries grape sapling phase: sapling -> discrete fruiting bottom/top pair. */
public final class AromaGrapeSaplingBlock extends BushBlock implements BonemealableBlock, EntityBlock {
    private final String cropId;
    public AromaGrapeSaplingBlock(BlockBehaviour.Properties properties, String cropId) { super(properties); this.cropId = cropId; }

    private Block block(String suffix) {
        return BuiltInRegistries.BLOCK.get(ResourceLocation.fromNamespaceAndPath(BreweryDelight.MOD_ID, cropId + suffix));
    }
    @Override public boolean isRandomlyTicking(BlockState state) { return true; }
    @Override protected void randomTick(BlockState state, ServerLevel level, BlockPos pos, RandomSource random) { if (random.nextInt(5) == 0) grow(level, pos); }
